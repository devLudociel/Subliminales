...
export const POST: APIRoute = async ({ request }) => {
  try {
    const { orderId, reason, details, userUid, userEmail } = await request.json();

    if (!orderId || !reason) return json({ error: 'Faltan datos requeridos' }, 400);

    const reasonLabel = REASONS[reason] ?? reason;
    ...
    try {
      ...
      await resend.emails.send({
        ...
        html: `<!DOCTYPE html><html lang="es"><body style="font-family:sans-serif;padding:24px;">
          ...
          <p><strong>Pedido:</strong> #${shortOrder} (${orderId})</p>
          <p><strong>Usuario:</strong> ${userEmail ?? 'No autenticado'}</p>
          ...
          html: `<!DOCTYPE html><html lang="es"><body style="font-family:sans-serif;padding:24px;background:#fef0f4;">
            <table width="600" style="max-width:600px;margin:0 auto;background:#fff;border:2px solid #111;border-radius:12px;overflow:hidden;">
              ...
              <tr><td style="padding:32px;">
                ...
                <p>Hemos recibido tu solicitud de devolución para el pedido <strong>#${orderId.replace('cs_', '').replace('test_', '').slice(0, 10).toUpperCase()}</strong>.</p>
                <p><strong>Motivo:</strong> ${reasonLabel}</p>
